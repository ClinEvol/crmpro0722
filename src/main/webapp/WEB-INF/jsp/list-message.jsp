                ...
                <div class="mail-box-header">
                    ...
                    <div class="mail-tools tooltip-demo m-t-md">
                        ...
                        <div class="btn-group pull-right">
                            <a href="/message/list?pageNum=${page.pageNum-1}&keyword=${keyword}" class="btn btn-sm btn-white <c:if test="${page.pageNum-1<1}">disabled</c:if> "><i class="glyphicon glyphicon-chevron-left"></i></a>
                            <a href="/message/list?pageNum=${page.pageNum+1}&keyword=${keyword}" class="btn btn-sm btn-white <c:if test="${page.pageNum+1>page.pages}">disabled</c:if> "><i class="glyphicon glyphicon-chevron-right"></i></a>
                            </button>
                    ...
                    <table class="table table-hover table-mail">
                        ...
                        <c:forEach items="${list}" var="message">
                            <tr class="unread">
                                ...
                                <td class="mail-ontact"><a href="/message/show?id=${message.msgid}">
                                    <c:if test="${message.title.length()>15}">${message.title.substring(0,15)}</c:if>
                                    <c:if test="${message.title.length()<=15}">${message.title}</c:if>
                                </a>
                                    <c:if test="${message.status==1}">
                                    <span class="label label-danger pull-right">未读</span>
                                    </c:if>
                                </td>
                                <td class="mail-subject"><a href="/message/show?id=${message.msgid}">
                                    <c:if test="${message.content.length()>30}">
