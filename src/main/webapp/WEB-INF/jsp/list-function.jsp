...
<body class="gray-bg">
    ...
    <div class="ibox float-e-margins">
                   <div class="ibox-content">
                        <div class="row">
                            <form method="post" action="/function/list">
                            ...
                            <div class="col-sm-2">
                                <select name="type" class="selectpicker form-control">
                                    ...
                                    <option value="1" <c:if test="${type==1}">selected</c:if> >功能名称</option>
                                    <option value="2" <c:if test="${type==2}">selected</c:if> >模块名称</option>
                                    <option value="3" <c:if test="${type==3}">selected</c:if> >需求名称</option>
                                    <option value="4" <c:if test="${type==4}">selected</c:if> >项目名称</option>
                                </select>
                            </div>
                            ...
                            <div class="col-sm-2 text-right">
                            		<a href="/function/tosave" class="btn btn-sm btn-primary" type="button"><i class="fa fa-plus-circle"></i>添加功能</a>
                            	</div>
                            </form>
                        </div>
                            ...
                            <table class="table table-striped list-table">
                                ...
                                <c:forEach items="${list}" var="function">
                                    <tr>
                                        ...
                                        <td>
	                                        	<a href="show-function.jsp"><i class="glyphicon glyphicon-eye-open  text-navy"></i></a>
                                    ...
                                    <c:if test="${fn:length(list)==0}">
                                        <tr><td colspan="10"class="text-center">没有找到相关的数据</td></tr>
                                    </c:if>
                       ...
                       <div class="row">
                           ...
                           <div class="col-sm-7 text-right">
                               <div class="btn-group">
                                   <a href="/function/list?pageNum=${page.pageNum-1}&keyword=${keyword}&type=${type}" class="btn btn-sm btn-white <c:if test="${page.pageNum-1<1}">disabled</c:if> "><i class="glyphicon glyphicon-chevron-left"></i></a>
                                   <c:forEach begin="1" end="${page.pages}" varStatus="index">
                                       <a href="/function/list?pageNum=${index.index}&keyword=${keyword}&type=${type}" class="btn btn-sm btn-white <c:if test="${page.pageNum==index.index}">active</c:if> ">${index.index}</a>
                                   </c:forEach>
                                   <a href="/function/list?pageNum=${page.pageNum+1}&keyword=${keyword}&type=${type}" class="btn btn-sm btn-white <c:if test="${page.pageNum+1>page.pages}">disabled</c:if> "><i class="glyphicon glyphicon-chevron-right"></i></a>
                               </div>
                           </div>
                       </div>
